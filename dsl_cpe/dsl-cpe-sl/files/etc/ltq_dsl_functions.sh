...
get_fw_type() {
    ...
    elif [ "$CONFIG_PACKAGE_DSL_CPE_API_DANUBE" = "1" ]; then
...
validate_fw_bits() {
    ...
    if [ "$wan_mode" != "AUTO" -a "$wan_mode" = "VDSL" ]; then
        ...
        xTSE=$ADSL_MODE
        /usr/sbin/status_oper -u -f /etc/rc.conf SET "adsl_phy" "ADSL_MODE" $ADSL_MODE "xDSL_MODE" "vdsl2"
        return
    fi
...
# nVDSL_TC_Mode - 1 - ATM, 2 - PTM, 4 - Auto
get_phy_tc_info() {
   wan_mode="NOT_DSL"
   nDSL_Mode="0 1"
   nADSL_TC_Mode="1"
